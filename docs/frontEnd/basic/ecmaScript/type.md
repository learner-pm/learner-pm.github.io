...
`const`声明得变量无法修改，所以一般把这个变量作为常量对待,如 **PI**。

当给变量赋值为一个对象时，`const`声明的变量储存的是一个`指针`，这个指针指向对象所处得`堆`位置，**指针不能修改，但可修改对象内部属性**。
...
`let`需要先声明，再赋值（保持先声明，再赋值的习惯利于写出含有更少的 bug 的代码）,同时会形成块级作用域。
...
拷贝可用方法`Object.assign();`,展开运算符`...`。数组的`Array.from()`。
